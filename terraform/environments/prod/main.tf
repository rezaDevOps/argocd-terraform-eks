...
module "argocd_terraform_prod" {
  ...
  # Production settings (HA and performance focused)
  single_nat_gateway = false  # HA required for production
  node_group_instance_types = ["m5.xlarge", "m5.large"]
  
  gitops_repo_url = var.gitops_repo_url
  domain_name     = var.domain_name
  
  # Enable all addons for production
